...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = kadence()->get_asset_version( $css_dir . $data['file'] );
	...
	public function generate_base_css() {
		...
		$css->add_property( 'padding', $this->render_responsive_measure( kadence()->option( 'header_logo_padding' ), 'desktop' ) );
		$css->start_media_query( $media_query['tablet'] );
		$css->set_selector( '.site-branding' );
		$css->add_property( 'padding', $this->render_responsive_measure( kadence()->option( 'header_logo_padding' ), 'tablet' ) );
		$css->stop_media_query();
		$css->start_media_query( $media_query['mobile'] );
		$css->set_selector( '.site-branding' );
		$css->add_property( 'padding', $this->render_responsive_measure( kadence()->option( 'header_logo_padding' ), 'mobile' ) );
		$css->stop_media_query();
		...
		if ( class_exists( 'woocommerce' ) ) {
			...
			$css->set_selector( '.woocommerce ul.products li.product .product-details .price' );
	...
	public function generate_editor_css() {
		...
		$css->add_property( '--global-palette-btn-bg-hover', $this->render_color( kadence()->sub_option( 'buttons_background', 'hover' ) ) );
		// Buttons.
		$css->set_selector( '.editor-styles-wrapper .wp-block-button .wp-block-button__link' );
		$css->render_font( kadence()->option( 'buttons_typography' ), $css );
		$css->add_property( 'border-radius', $this->render_range( kadence()->option( 'buttons_border_radius' ), 'desktop' ) );
		$css->add_property( 'padding', $this->render_responsive_measure( kadence()->option( 'buttons_padding' ), 'desktop' ) );
